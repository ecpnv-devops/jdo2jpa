...
- [ ] 6.6 Confirm both turnover entities receive the intended integration, excluded BackgroundCommandsOrchestration is unchanged, and actual per-module output is repeat-run stable without redundant effective listener registrations.
  ...
  - Full-file repeat stability remains unconfirmed globally: three other sampled modules change on repeat, with every candidate delta reproduced on the baseline (unused imports and two test-subclass discriminator annotations).
  ...
  - Supplying genuine generated QueryDSL sources enables cleanup of the two unchanged-input capex files in a controlled probe; a post-annotation-processing cleanup stage is proposed but not approved or validated, as recorded in `docs/entity-listener-idempotence-follow-up.md`.
