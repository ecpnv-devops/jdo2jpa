...
public final class AddCausewayEntityListener extends ScanningRecipe<EntityTypeResolver> {
    ...
    @Override public String getDisplayName() { return "Preserve Causeway entity listener integration"; }
    @Override public String getDescription() { return "Add a missing listener without duplicating effective superclass listeners or overriding explicit declarations."; }
    @Override public EntityTypeResolver getInitialValue(ExecutionContext ctx) { return new EntityTypeResolver(); }
    @Override public TreeVisitor<?, ExecutionContext> getScanner(EntityTypeResolver resolver) { return resolver.scanner(); }
    ...
    private boolean inherited(J.ClassDeclaration cd, J.CompilationUnit cu, EntityTypeResolver resolver,
        ...
        if (annotation(cd, EXCLUDE) != null || cd.getExtends() == null) { return false; }
    ...
    private boolean effective(String name, JavaType.FullyQualified known, J.CompilationUnit cu,
        ...
        if ("java.lang.Object".equals(name)) { return false; }
        if (!visiting.add(name)) { throw unresolved(entity, name); }
        try {
            J.ClassDeclaration source = resolver.declaration(cu, name);
            if (source != null) {
                J.Annotation own = annotation(source, LISTENERS);
                if (own != null && sourceListener(own, resolver.owner(cu, name), entity)) { return true; }
            ...
            if (type == null) { throw unresolved(entity, name); }
            for (JavaType.FullyQualified a : type.getAnnotations()) {
                if (LISTENERS.equals(a.getFullyQualifiedName())) {
                    if (!(a instanceof JavaType.Annotation values)) { throw unresolved(entity, name); }
                        ...
                        if (value instanceof JavaType.Annotation.ArrayElementValue array) {
                            ...
                            if (references != null) {
                                for (JavaType reference : references) {
                                    if (TypeUtils.isOfClassType(reference, listenerClass)) { return true; }
                                }
                            }
            ...
            if (type.getAnnotations().stream().anyMatch(a -> EXCLUDE.equals(a.getFullyQualifiedName()))) { return false; }
            JavaType.FullyQualified parent = type.getSupertype();
            if (parent == null) { throw unresolved(entity, name); }
    ...
    private boolean sourceListener(J.Annotation annotation, J.CompilationUnit cu, String entity) {
        if (annotation.getArguments() == null) { return false; }
        for (Expression value : annotation.getArguments()) {
            if (containsListener(value, cu, entity)) { return true; }
        }
        return false;
    }

    private boolean containsListener(Expression value, J.CompilationUnit cu, String entity) {
        if (value instanceof J.Assignment assignment) { return containsListener(assignment.getAssignment(), cu, entity); }
        ...
        if (value instanceof J.FieldAccess field && "class".equals(field.getSimpleName())) {
            ...
            if (type != null) { return listenerClass.equals(type.getFullyQualifiedName()); }
            ...
            if (listenerClass.equals(name)) { return true; }
            for (J.Import imprt : cu.getImports()) {
                if (!imprt.isStatic() && imprt.getTypeName().endsWith("." + name)) {
                    return listenerClass.equals(imprt.getTypeName());
                }
            }
            if (name.contains(".") && Character.isLowerCase(name.charAt(0))) { return false; }
            throw unresolved(entity, name);
        }
        if (value instanceof J.Empty) { return false; }
