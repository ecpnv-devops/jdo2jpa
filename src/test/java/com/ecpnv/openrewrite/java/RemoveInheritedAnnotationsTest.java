...
class RemoveInheritedAnnotationsTest extends BaseRewriteTest {
    ...
    @Test
    void removesColumnAddedToParentEarlierInTheRecipeSequence() {
        removesColumnAddedToParent(false);
    }

    @Test
    void removesColumnWhenChildIsVisitedBeforeChangedParent() {
        removesColumnAddedToParent(true);
    }

    private void removesColumnAddedToParent(boolean childFirst) {
        var parent = java(
                """
                                @Deprecated
                                class Parent {
        ...
        rewriteRun(
                spec -> spec.recipes(
                        new AddAnnotationConditionally("@Deprecated", null,
                                "javax.persistence.DiscriminatorColumn", "@DiscriminatorColumn",
                                AddAnnotationConditionally.DeclarationType.CLASS,
                                null, null, null, null, null),
                        new RemoveInheritedAnnotations(Set.of("javax.persistence.DiscriminatorColumn")))
                        .cycles(3).expectedCyclesThatMakeChanges(2),
                childFirst ? child : parent,
                childFirst ? parent : child);
    }

    @Test
    void alreadyNormalizedHierarchyIsStable() {
        rewriteRun(
                spec -> spec.recipe(new RemoveInheritedAnnotations(Set.of("javax.persistence.DiscriminatorColumn")))
                        .cycles(3).expectedCyclesThatMakeChanges(0),
                java(
                        """
                                import javax.persistence.DiscriminatorColumn;
